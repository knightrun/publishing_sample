...
export default () => {
    ...
    function getDataForFile(file){
        ...
        if(fs.existsSync(jsonUrl)){
            ...
            if(file.history[0].indexOf('@guide') !== -1){
                ...
                const guideData = options.multilingual ? loadPreviewLanguagePath(guideGlob) : loadPreviewPath(guideGlob);
                ...
                const otherData = options.multilingual ? loadPreviewLanguagePath(otherGlob) : loadPreviewPath(otherGlob);
    ...
    gulp.task('product_all', gulp.parallel(productStaticCss, productCss, productImage, productFont, productBuildSample, productBuildGuide, productBuildOther));
    gulp.task('product_build', gulp.parallel(productStaticCss, productCss, productImage, productFont));
